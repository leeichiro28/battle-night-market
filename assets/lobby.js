...
// 五手勢對戰專用的規則說明——跟 rps5.js 裡對戰畫面用的 RULE_EXPLAIN 是同一份內容，
// 這裡是活動還沒開打、玩家在報名頁/大廳先看到的版本，改其中一邊記得同步改另一邊。
const RPS5_RULE_EXPLAIN = {
  bomb: ["隱藏第六手勢", "第 3 回合起，每回合約有 15% 機率額外開放這隻隱藏手勢，開放時選擇列會多一個「炸彈」選項可以選。剋制關係:炸彈 勝 石頭、剪刀;炸彈 敗 布、蜥蜴;炸彈 對 史波克 是特殊平局，雙方不掉血。"],
  field_mod: ["戰場修飾骰", "開局隨機決定這一局固定生效的特殊效果，3 選 1:磐石戰場(靠石頭獲勝時傷害+1)、手速戰場(究極手勢這局可用2次)、疾風戰場(思考時間縮短到30秒)。"],
  item_die: [
    "道具骰",
  ...
  stance: ["出招姿態", "出招前可以先公開宣告這局「偏攻擊」或「偏防禦」，純粹是情報，不會直接影響傷害，對方看得到但不知道真假;宣告會跟手勢一起在戰報揭曉，唬多了容易被〈讀心值〉或對方肉眼抓到規律。"],
  combo: ["連擊值", "系統會記錄你最近用哪個手勢獲勝。連續 3 局都用同一手勢獲勝，額外 +2 傷害，並跳出「連段技發動！」;之後繼續用同一招連勝下去，每一局都會持續拿到加成。要不要賭一把繼續出同一招，風險自負。"],
  mindread: ["讀心值", "系統偷偷統計對方整場出招的習慣分布，如果你選中「剋制對方最常出的那招」並獲勝，額外 +1 傷害，並跳出「讀心成功！」。開啟這項規則時，對戰畫面也會顯示對方的即時出招傾向統計，觀眾也看得到同一份統計。"],
  momentum: ["氣勢系統", "連勝 2 局起，下一擊額外 +1 傷害，氣勢會一直維持到輸掉一局為止;連續落敗 2 局後，如果靠獲勝逆轉，那一擊的傷害會直接翻倍(背水一戰)。"],
  mutation: ["手勢突變", "連續 3 回合都出同一個手勢，下一回合系統會把那個手勢從選項裡鎖住，逼你換一招，防止靠「無腦一直出同一招」硬撐。"],
  hand_limit: [
    "手牌制",
    "整局把 18 張手勢牌(石頭/布/剪刀各4、蜥蜴/史波克各3)洗好，固定發 4 張當手牌，出招畫面用卡片呈現。出一張補一張(照洗好的固定順序發，不是隨機抽)，只看得到自己的手牌。炸彈不算在手牌裡，究極手勢底層一樣要補牌;牌堆抽完後手牌會越打越少，打完會解除限制改回自由選擇。開啟時手勢突變自動停用。",
  ],
  bo_mode: ["BO制", "整場對戰拋開 HP 累加機制，改成每回合直接分出這一分的勝負，率先取得 3 分的一方贏得整場;打到 2:2 時觸發「賽末點！」提示。以下基礎規則裡跟 HP / 局數相關的敘述，本場一律不適用。"],
  dual_hand: ["雙手出招", "落後的一方整場限用 1 次「雙手符」，可以同時出兩個手勢，只要其中一個贏過對方的招就算贏。觸發資格:HP 制下自己 HP ≤8 才能使用;BO制下自己的局分落後對方才能使用。限用 1 次，是殘局翻盤手段，不會變成必勝招。"],
};
  ...
  if (ev.game_type === "dice") {
    ...
    Object.keys(rules)
      ...
      .forEach((k) => {
        ...
        if (item) {
          html += `<div class="rule-item"><b style="color:var(--ink);">${meta ? ui.icon(meta.icon) + " " : ""}${item[0]}</b><div class="rule-item-body">${item[1]}</div></div>`;
        }
      });
  } else {
    // 五手勢對戰:一般場 BO3(先2勝)、總冠軍賽 BO5(先3勝)——這裡還沒配到實際對戰(match)，
    // 所以不知道玩家最後會不會打進冠軍賽，先概括說明兩種賽制都有，跟 rules.html / rps5.js 對戰畫面裡的用詞保持一致。
    const rules = ev.rules || {};
    const bombOn = !!rules.bomb;
    html += `<p>一般對戰採 BO3 賽制(先拿下 2 局者勝，最多打到第 3 局);總冠軍賽賽制較長，維持 BO5(先拿下 3 局者勝，最多打到第 5 局)。雙方各有 15 點 HP，先讓對方 HP 歸零的人拿下這一局，每進入新一局，雙方 HP 會全部回滿。</p>`;
    html += `<p>每回合 45 秒內選一個手勢:石頭 / 布 / 剪刀 / 蜥蜴 / 史波克${bombOn ? "(本場另有機率額外開放隱藏手勢「炸彈」，見下方說明)" : ""}。超時未選視為該回合落敗，雙方都超時則平手不掉血。</p>`;
    html += `<p>石頭勝剪刀、蜥蜴;布勝石頭、史波克;剪刀勝布、蜥蜴;蜥蜴勝史波克、布;史波克勝剪刀、石頭。出了相同的手勢就是平手。</p>`;
    html += `<p>每人每一局都有 1 張「究極手勢」卡:出牌保證獲勝該回合，除非對方同一回合也出究極手勢，此時雙方抵銷、判定平手。</p>`;
    html += `<p>當你的 HP ≤5 時，獲勝的那一擊傷害會翻倍，適合絕地反擊。BO3 打到 1:1、BO5 打到 2:2 時會有「賽末點」提示。</p>`;
    Object.keys(rules)
      .filter((k) => rules[k])
      .forEach((k) => {
        const item = RPS5_RULE_EXPLAIN[k];
        const meta = ui.RULE[k];
        if (item) {
          html += `<div class="rule-item"><b style="color:var(--ink);">${meta ? ui.icon(meta.icon) + " " : ""}${item[0]}</b><div class="rule-item-body">${item[1]}</div></div>`;
        }
